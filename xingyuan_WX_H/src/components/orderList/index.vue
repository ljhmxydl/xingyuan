  ...
  <div class="components_order_list">
    ...
    <div class="weui_cell ctph-order-list-header">
      <div class="weui_cell_bd weui_cell_primary">
        <img
          :src="order.product_image ? order.product_image : 'http://o9s1f7266.bkt.clouddn.com/ant_logo.png?imageView2/2/w/30/h/30'"/>
    ...
    <div class="weui_cell vux-tap-active" @click="$emit('on-click-order')">
      <div class="weui_cell_bd weui_cell_primary">
        <template v-for="item in order.order_detail">
          <flexbox>
            <flexbox-item :span="7"><p class="ellipsis" style="font-size:16px;margin-bottom:.3rem;">
              {{item.product_name}}({{item.product_specification}})</p></flexbox-item>
            <flexbox-item :span="2"><p class="ellipsis" style="text-align:right;">×{{item.buy_amount}}</p>
            </flexbox-item>
            <flexbox-item :span="3"><p class="ellipsis" style="text-align:right;padding:0 .75rem">
              ￥{{item.product_price}}</p></flexbox-item>
          </flexbox>
        </template>
        <div>
    ...
    <template v-if="order.order_status === 1">
      <div class="weui-form-preview__ft">
        <a href="javascript:;" class="weui-form-preview__btn weui-form-preview__btn_default"
           @click="$emit('on-click-cancel')">取消订单</a>
        <a href="javascript:;" class="weui-form-preview__btn weui-form-preview__btn_primary"
           @click="$emit('on-click-pay')">去支付</a>
      </div>
    </template>
